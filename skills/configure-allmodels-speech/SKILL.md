...
7. If a tool result contains `plugin_update`, mention it briefly after completing the current setup step and remind the user to restart the Hermes gateway after the update completes. Never interrupt an email or verification-code request with the notice, and never install an update without an explicit user request.
